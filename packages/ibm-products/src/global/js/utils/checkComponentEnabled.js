...
export const logDeprecated = (component, name) => {
...
//
// NOTE: this lives in its own module (rather than being attached to the
// package settings in settings.js) so that consumers who never import a
// feature-flagged component do not pay the bundle cost of the Canary
// placeholder and its dependencies.
export const checkComponentEnabled = (component, name) => {
  if (component.render) {
    // The component is a forward-ref, so make a stub forward-ref.
    const forward = React.forwardRef((props, ref) => {
      logDeprecated(component, name); // may log don't care about result
